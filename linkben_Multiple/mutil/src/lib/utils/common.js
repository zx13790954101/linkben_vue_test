...
// 截取url的参数
export const urlGet = (key, isDecode) => {
  ...
  p.map(function (item) {
    ...
    if (temp[0] === key) {
      if (temp.length > 2) {
        ...
        v = isDecode ? decodeURI(temp[1]) : temp[1]
      }
    }
  })
  return v
...
export const isApp = () => {
  ...
  if (UserAgent.indexOf('hxsapp-version-') >= 0) {
    return true
  } else {
    return false
  }
...
// APP>= 3.1.1 跳转订单列表 sortype： 1-待付款，2-待发货，3-待收货，4-已完成，0-全部
export const goOrderlist = (sortype) => {
  window.location.href = 'https://hxsapp_orderlist?sortype=' + sortype
}
//团购情H5跳去新的确认订单
export const groupOrder = ({ skuId, num, groupid, isgroup }) => {
  const Version = getHxsAppVersion()
  if ((compareAppVersion(Version, '3.9.5') || Version === '3.9.5') && Version) {
    window.location.href = 'https://hxsapp_order_group_buy?data=[{"sku_id":"' + skuId + '","num":"' + num + '"}]&groupid=' + groupid + '&isgroup=' + isgroup
  } else {
    Toast('请更新APP版本')
  }
}
