...
var config = {
  ...
  prefixUrl:
    process.env.NODE_ENV === "production"
      ? "https://app.hxsapp.com/hxswap/"
      : window.location.origin + "/hxswap/",
...
function outSideShare(Info) {
  // $.getJSON('https://act.hxsapp.com/wechat/Api/getWxSignature?callback=?', { url: window.location.href, gzh: "hxsapp"}, function(data) {
  //   var data = data.data
  //   seconDaryShare(
...
//微信环境判断_ch
function isweixin() {
  const uax = window.navigator.userAgent.toLowerCase();
  if (uax.indexOf("micromessenger") > -1) {
    return true;
  } else {
    return false;
  }
